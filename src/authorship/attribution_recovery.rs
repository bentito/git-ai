...
fn recover_known_human_remaining_lines(
    ...
    let parent_authorship_log = if parent_sha == "initial" {
        ...
        crate::git::notes_api::read_authorship_v3(repo, parent_sha).ok()
    };

    let mut recoveries = Vec::new();
    for (file_path, ranges) in untracked_ranges_by_file {
        let Some(human_id) =
            unique_known_human_for_file(authorship_log, parent_authorship_log.as_ref(), &file_path)
        else {
            continue;
        };
        if let Some(parent_log) = parent_authorship_log.as_ref()
            && let Some(record) = parent_log.metadata.humans.get(&human_id)
        {
            authorship_log
                .metadata
                .humans
        ...
        recoveries.push((file_path, ranges, human_id));
    }

    for (file_path, ranges, human_id) in recoveries {
...
fn unique_known_human_for_file(
    ...
    for log in parent_authorship_log
        .into_iter()
        .chain(std::iter::once(authorship_log))
    {
        let Some(file_attestation) = log
            .attestations
            .iter()
            .find(|att| att.file_path == file_path)
        else {
        ...
        for entry in &file_attestation.entries {
            ...
            match known_human.as_deref() {
                None => known_human = Some(entry.hash.clone()),
